...
function renderAllAdmin(){
  ...
  renderTable("#table-messages tbody", ADMIN_DATA.messages, `<tr><td colspan="4" class="muted">No messages yet.</td></tr>`, m => `<tr><td>${new Date(m.date).toLocaleDateString("en-GB")}</td><td><strong>${escapeHTML(m.name)}</strong><br><span class="muted">${escapeHTML(m.email)}</span></td><td>${escapeHTML(m.kind === "donation-intent" ? `Donation intent — MWK ${m.amount} (${m.method})` : (m.subject||"") + (m.message ? `: ${m.message}` : ""))}</td><td><button class="reply-btn" data-email="${escapeHTML(m.email)}" data-name="${escapeHTML(m.name)}" data-subject="${escapeHTML(m.subject || "Your enquiry to Umodzi New Community")}">Reply</button> <button class="del-btn" data-id="${m.id}" data-type="submissions">Delete</button></td></tr>`);
